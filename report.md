...
*   **Gate Bridging**: The engine explicitly detects nodes near specific campus gates. `ks_gate` (Katwaria Sarai gate) nodes inside and outside the boundary are bridged if within a **30m** radius. `js_gate` (Jia Sarai) is bridged similarly using a **50m** search radius.
...
## Results: occupancy curves, arrival/departure distributions, summary statistics
`analyze_building_occupancy.py` acts as the primary analytical tool for the simulation.
*   **Mechanism**: Iterates over schedules to evaluate the population count within specific building archetypes at exact 30-minute intervals (from 06:00 to 24:00).
*   **Classification**: Categorizes rooms via substrings. 
    *   *LHC*: matches `LH ` or `LHC`.
